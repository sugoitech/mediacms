...
if [ -f "ffmpeg-release-amd64-static.tar.xz" ]; then
    echo "ffmpeg is existing skip Downloading"
else
...
read -r -p "Enter portal URL, or press enter for localhost : " FRONTEND_HOST
read -r -p "Enter portal name, or press enter for 'MediaCMS : " PORTAL_NAME
...
echo "Creating python virtualenv on $HOME_DIR"
...
if [ "$FRONTEND_HOST" != "localhost" ]; then
    ...
    openssl dhparam -rand rand.seed -out /etc/nginx/dhparams/dhparams.pem 4096
    systemctl restart nginx
else
